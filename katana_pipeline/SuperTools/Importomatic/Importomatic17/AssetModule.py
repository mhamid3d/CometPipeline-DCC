__all__ = [
    'AssetModule', 'AssetTreeChild']

from Constants import *
from Katana import NodegraphAPI
import os, logging

log = logging.getLogger('Importomatic.AssetModule')
...
class AssetModule(object):
    ...
    def GetAssetType(node):
        typeParam = node.getParameter('__importomaticType')
        if not typeParam:
            return node.getType()
    ...
    def SetAssetType(node, typeName):
        ...
        if not typeParam:
            node.getParameters().parseXML(
                "\n            <group_parameter>\n             <string_parameter name='__importomaticType'/>\n            </group_parameter>\n            ")
            typeParam = node.getParameter('__importomaticType')
            typeParam.setHintString(str({'widget': 'null'}))
        typeParam.setValue(typeName, 0)

    @staticmethod
    def GetCustomAssetName(node, paramName='__importomaticName'):
        if not node:
            return
        else:
    ...
    @staticmethod
    def SetCustomAssetName(node, assetName, paramName='__importomaticName'):
        param = node.getParameter(paramName)
    ...
    def addToContextMenu(self, menu, importomaticNode, node):
        pass
