...
log = logging.getLogger('Importomatic.Editor')


class ImportomaticEditor(QtWidgets.QWidget):
    ...
    def __init__(self, parent, node):
        ...
        mainLayout.addWidget(self.__tree)
        mainLayout.addWidget(UI4.Widgets.VBoxLayoutResizer(self.__tree, 160, 128))
        self.__headerLabels = [
            'Asset Name', 'Type', 'Version', 'Resolved Version', 'Status']
        ...
        for eventType, _eventID, kwargs in args:
            if eventType == 'parameter_finalizeValue':
                ...
                if self.__nodeIsMine(node):
                    update = True
            elif eventType in ('port_connect', 'port_disconnect'):
            ...
            elif eventType == 'node_setBypassed':
                node = kwargs['node']
                if self.__nodeIsMine(node):
                    update = True
            elif eventType == 'node_renameOutputPort':
                if kwargs['node'] == self.__node:
                    update = True
            elif eventType == 'nodegraph_defaultPluginsChanged':
                update = True
            elif eventType == 'cacheManager_flush':
                update = True
            if update:
                break
    ...
    def __dragMoveEventCallback(self, event, parent, index, callbackRecord):
        ...
        if event.mimeData().hasFormat('importomatic/assettree'):
            if not parent:
                return
    ...
    def __listKeyPressCallback(self, event):
        ...
        if event.isAccepted():
            ...
            if key == QtCore.Qt.Key_Delete:
                ...
                items = [x for x in self.__tree.selectedItems() if
                         x.getItemData()['type'] == 'assetTree' and x.getItemData()['assetItem'].isIgnorable()
                         ]
                if not items:
                    return
                ToggleIgnoreItemsAction(None, self.__node, items,
                                        not items[0].getItemData()['assetItem'].isIgnored()).go(True)
            return
...
class LoadDuplicateWarningDialog(QtWidgets.QDialog):
    ...
    def setProductLabel(self, product):
        self.__productLabel.setText('<b>%s</b> already exists in this Importomatic.' % product)
