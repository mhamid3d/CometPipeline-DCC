...
log = logging.getLogger('Importomatic.Node')
NodegraphAPI.AddNodeFlavor('Importomatic', '3d')


class ImportomaticNode(NodegraphAPI.SuperTool):

    def __init__(self):
        self.hideNodegraphGroupControls()
        self.addOutputPort('default')
        self.getParameters().parseXML(
            '\n                <group_parameter>\n                    <string_parameter name="__nodePanel" value="Importomatic"/>\n                    <number_parameter name="__pluginRevision" value="1"/>\n                </group_parameter>')
    ...
    def getFilterAssetStackOutput(self, outputName, build=False):
        ...
        if not port:
            ...
            if not node:
                if build:
                    raise TypeError('Internal Importomatic graph error')
                return None
